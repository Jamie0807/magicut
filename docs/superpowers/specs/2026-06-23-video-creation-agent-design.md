...
本设计定义 Magicut 第一版智能视频创作流程：用户提供创作提示词和本地视频片段，系统通过本地优先的智能体流程生成一个可编辑的 `VideoProject JSON`。编辑器读取该 JSON 后展示视频、配音、字幕、音乐四类轨道。
...
智能体运行在 Electron 本地 Node 能力侧。Vue renderer 只负责提交创作意图、展示进度、承接人工确认和打开生成的工程。
...
- `apps/desktop/renderer`
    - Vue 3 只消费 project loading API 和 `VideoProject` 派生出的 UI view model。
