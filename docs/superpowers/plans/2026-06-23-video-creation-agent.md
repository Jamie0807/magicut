...
**Architecture:** 新增 `packages/video-project` 管理工程 JSON schema，新增 `packages/video-agent` 管理 LangGraph 编排和工具接口，Electron main/preload 承担本地文件、SQLite、媒体工具和外部 API 访问，Vue renderer 只消费受控 API 和 view model。

**Tech Stack:** TypeScript, pnpm workspace, Zod, Vitest, tsdown, dotenv, Electron Forge, Vue 3, LangChainJS, LangGraphJS, Ark/OpenAI-compatible ChatOpenAI, 火山引擎 seed-tts-2.0 WebSocket, ffmpeg, ffprobe, node:sqlite.
...
- 文档只出现 Magicut、Vue、`@magicut/*` 和当前仓库真实 `.vue` 路径。
...
- package name 使用 `@magicut/video-project`。
...
pnpm --filter @magicut/video-project build
pnpm --filter @magicut/video-project lint
...
- Modify: `apps/desktop/magicut.env.d.ts`
...
- preload 暴露 `window.magicutAPI.videoProject`。
- main 侧读写 JSON 前后都调用 `@magicut/video-project` 校验。
...
pnpm --filter @magicut/video-agent exec tsc --noEmit
```
...
```bash
pnpm --filter @magicut/video-agent test:run
pnpm --filter @magicut/video-agent build
pnpm --filter @magicut/video-agent lint
```
...
- Modify: `apps/desktop/renderer/pages/EditorScreen.vue`
- Modify: `apps/desktop/renderer/components/editor/TimelinePanel.vue`
...
```bash
pnpm --filter @magicut/desktop test:run
pnpm exec eslint "apps/desktop/renderer/mappers/video-project-to-editor.ts" "apps/desktop/renderer/pages/EditorScreen.vue" "apps/desktop/renderer/components/editor/TimelinePanel.vue"
pnpm exec tsc --noEmit -p "apps/desktop/tsconfig.json"
```
...
- Modify: `apps/desktop/renderer/components/create/CreateInputPanel.vue`
- Modify: `apps/desktop/renderer/pages/HomePage.vue`
- Create: `apps/desktop/renderer/components/create/CreateAgentProgress.vue`
- Modify: `apps/desktop/client/preload.ts`
- Modify: `apps/desktop/client/main.ts`
...
- UI 展示：
    ...
    - failed / 已失败
- preload 暴露：
    - `window.magicutAPI.videoAgent.start()`
    - `window.magicutAPI.videoAgent.onEvent()`
    - `window.magicutAPI.videoAgent.approve()`
    - `window.magicutAPI.videoAgent.cancel()`
