...
describe('video project store', () => {
    ...
    it('lists saved projects sorted by latest update time and ignores invalid files', async () => {
        ...
        await writeFile(
            path.join(tempDirectory, 'broken.magicut.json'),
            '{ "schemaVersion": "1.0.0" }',
            'utf8'
        );
        await writeFile(
    ...
    it('exposes project listing and deletion through IPC and preload contracts', async () => {
        const { registerVideoProjectIpc } = await import(
            '../client/video-project-ipc'
        );
        const { videoProjectIpcChannels } = await import(
            '../shared/video-project-channels'
        );
